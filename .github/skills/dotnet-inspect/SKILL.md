...
# >-  linebreaks stripped and do not append a newline to the end of the value
# |-  linebreaks preserved and do not append a newline to the end of the value
description: >
    Query .NET APIs across NuGet packages, platform libraries, and local files. Search for types, list API surfaces, compare 
    and diff versions, find extension methods and implementors. 
    Use whenever you need to answer questions about .NET library contents.
---
...
*   Query 
    ...
    *   platform libraries (System.*, Microsoft.AspNetCore.*)
    
    *   local .dll/.nupkg files.
...
*   broken code
    ...
    ```shell
    diff --package Foo@old..new` first, then `member`
    ```
